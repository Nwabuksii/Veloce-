...
export default function ScribeUploadPage() {
  ...
  async function handleBlockNext() {
    ...
    if (cleanTopics.length < 3) {
      setError("Add at least 3 topics for this block.");
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error?.formErrors?.[0] || data.error || "Could not create block");
      ...
      <div className="app-container" style={{ maxWidth: 620 }}>
        ...
        {step === 1 && (
          <div style={{ marginTop: "1.5rem" }}>
            <h2>
              <i className="fas fa-book" style={{ color: "#2a7de1" }}></i> Which course?
            </h2>
        ...
        {step === 2 && (
          <div style={{ marginTop: "1.5rem" }}>
            <h2>
              <i className="fas fa-layer-group" style={{ color: "#2a7de1" }}></i> Which block?
            </h2>
            <p style={{ color: "#5e7188", fontSize: "0.85rem", marginTop: "0.3rem" }}>{resolvedCourseLabel}</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem" }}>
                <label style={{ fontSize: "0.85rem", fontWeight: 500 }}>
                  ...
                  <input
                    ...
                    placeholder="Name this block anything you like"
                    style={inputStyle}
                  />
                </label>
                  ...
                  {topics.map((t, i) => (
                    <div key={i} style={{ display: "flex", gap: "0.5rem", marginBottom: "0.5rem" }}>
                      <input
                        ...
                        style={{ flex: 1, padding: "0.6rem", borderRadius: "0.7rem", border: "1px solid #d0dae8" }}
                      />
                      {topics.length > 3 && (
        ...
        {step === 3 && (
          <div style={{ marginTop: "1.5rem" }}>
            <h2>
              <i className="fas fa-cloud-upload-alt" style={{ color: "#2a7de1" }}></i> Upload your PDF
            </h2>
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem", marginTop: "1rem" }}>
              <input type="file" accept="application/pdf" onChange={(e) => setFile(e.target.files?.[0] || null)} required />

              {status && <p style={{ color: "#3b4c62" }}>{status}</p>}

...
export const GET = requireRole<RouteContext>(
  ...
  async (req: NextRequest, viewer, ctx) => {
    ...
    // Banned accounts vanish from view for everyone except admins (who
    // still need to see them for moderation) — same generic "not found"
    // rather than revealing the ban to a stranger browsing profiles.
    if (scribe.bannedAt && viewer.role !== "ADMIN") {
      return NextResponse.json({ error: "User not found" }, { status: 404 });
    }
